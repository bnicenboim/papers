...
tabular.table <- function(table,preheader="",linesinrows=c() ,linesincols=c(), tightcols=c(ncol(table))){
  ...
  begin <- paste("\\begin{tabular}{",cs," }\\thickline",sep="")
  ...
  header <- paste("Predictor & ",paste(colnames(table),collapse=" & "), "\\\\","\\hline",sep=" ")
  cat(header)
  cat("\n")
  for(i in 1:nrow(table)){
    if(i %in%linesinrows) {  #make a line after certain rows
      cat("[3pt] \\hline \\rule{0pt}{3ex}")
      cat("\n")
    }
